...
class EnrollmentModelView(ModelView):
    ...
    def get_query(self):
        return self.session.query(self.model).join(User, Enrollment.student_id == User.id).filter(User.role != 'admin')
    
    def get_count_query(self):
        return self.session.query(db.func.count('*')).select_from(self.model).join(User, Enrollment.student_id == User.id).filter(User.role != 'admin')
    
    form_args = {
        'student': {
            'query_factory': lambda: User.query.filter_by(role='student').all()
        }
    }
